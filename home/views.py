            ...
            if (uid == author_id):
                ...
                if visibility:
                    posts = Post.objects.filter(
                        visibility=visibility, author__author_id=author_id)
                else:
                    posts = Post.objects.filter(author__author_id=author_id)
        except AttributeError:
            posts = Post.objects.filter(
                visibility='PUBLIC', author__author_id=author_id)
...
class CommentLikes(APIView):
    ...
    def get(self, request, author_id, post_id, comment_id):
        ...
        object_url = author.profile_url + "/posts/" + \
            str(post_id) + "/comments/" + str(comment_id)
...
class InboxDetails(APIView, PageNumberPagination):
    ...
    def handle_local_or_remote_post(self, request):
        ...
        if new_post.is_valid():
            if not is_local_post:
                ...
                does_author_exist = len(
                    Author.objects.filter(author_id=author_id))

                # associate the post with an author
                Author.objects.create(
                    **new_post.validated_data['author']) if not does_author_exist else False
                new_post.validated_data['author'] = Author.objects.get(
                    author_id=author_id)
    ...
    def handle_local_or_remote_comment(self, request):
        ...
        is_local_comment = len(
            Comment.objects.filter(url_id=request.data['id']))
        ...
        if new_comment.is_valid():
            if not is_local_comment:
                ...
                Author.objects.create(
                    **new_comment.validated_data['author']) if not does_author_exist else False
                new_comment.validated_data['author'] = Author.objects.get(
                    author_id=author_id)
                access_comment = Comment.objects.create(
                    **new_comment.validated_data)
            else:
    ...
    def get(self, request, author_id):
        ...
        inbox_items = Inbox.objects.filter(
            associated_author__author_id=author_id)
        ...
        inbox_json = {"type": "inbox",
                      "author": current_author.profile_url, "items": inbox_list}
    ...
    def post(self, request, author_id):
        ...
        if request.data["type"] == "post":
            ...
            check_for_post = current_author.inbox_items.filter(
                inbox_item=new_post_dict)
        ...
        # COMMENT
            ...
            new_comment, new_comment_dict = self.handle_local_or_remote_comment(
                request)

            # checking that we're not adding duplicates in the inbox
            check_for_comment = current_author.inbox_items.filter(
                inbox_item=new_comment_dict)
        ...
        # LIKE
            ...
            # checking that we're not adding duplicates in the inbox
            check_for_like = current_author.inbox_items.filter(
                inbox_item=request.data)
            if len(check_for_like) > 0:
                return Response(status=status.HTTP_400_BAD_REQUEST)
            else:  # we haven't seen this like "post" before yet
                context_url = request.data["@context"] if "@context" in request.data.keys(
                ) else request.data["context"]
                Like.objects.create_like(
                    context_url, request.data["author"], request.data["object"])
        ...
        # FOLLOW
            ...
            does_follow_exist = Follow.objects.filter(
                author_object=request.data["object"]).filter(author_actor=request.data["actor"])

            # the follow does not exist (first time we're making a follow request)
            if len(does_follow_exist) == 0:
                new_follow = Follow.objects.create_follow(
                    request.data["actor"], request.data["object"])
            ...
            # the follow request existed before, now it either got accepted or denied
            elif len(does_follow_exist) > 0 and "state" in request.data.keys():
                old_follow = does_follow_exist[0]
                serializer = FollowSerializer(old_follow)
                old_follow_data = json.loads(json.dumps(
                    serializer.data))  # changing it to JSON dict

                # get the specific follow request from the inbox
                follow_from_inbox = current_author.inbox_items.filter(
                    inbox_item=old_follow_data)[0]

                if request.data["state"] == "Accepted":

                    # create a follower
                    current_author.followers_items.create(
                        author_info=json.loads(json.dumps(request.data["actor"])))
                    ...
                    old_follow_data = json.loads(json.dumps(
                        serializer.data))  # changing it to JSON dict
                    current_author.inbox_items.create(
                        inbox_item=old_follow_data)
